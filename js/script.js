...
handleContractUpdate(contract) {  
    ...
    this.log(`Contract update: ID=${contract.contract_id}, Status=${contract.status || 'unknown'}, Profit=${contract.profit || 'N/A'}`, 'debug');  

    if (this.config.takeProfitEnabled && this.checkDynamicExit(contract)) {  
        return;  
    }  

    // Enhanced condition: Handle both manual sell (is_sold) and auto-expiry (status === 'expired' or 'won'/'lost')  
    const isFinalized = contract.is_sold || contract.status === 'expired' || contract.status === 'won' || contract.status === 'lost';  

    if (isFinalized) {  
        const pnl = contract.profit !== undefined ? parseFloat(contract.profit) :  
                     (contract.payout !== undefined ? parseFloat(contract.payout) - parseFloat(this.activeContract.buyPrice) : 0);  
        ...
        if (isWin) {  
            ...
            this.consecutiveLosses = 0;  
            this.log(`Trade WON: +$${pnl.toFixed(2)} (Status: ${contract.status || 'sold'})`, 'success');  
        } else {  
            this.losses++;  
            this.currentStreak = this.currentStreak < 0 ? this.currentStreak - 1 : -1;  
            this.lastTradeResult = 'loss';  
            this.consecutiveLosses++;  
            this.log(`Trade LOST: -$${Math.abs(pnl).toFixed(2)} (Status: ${contract.status || 'sold'})`, 'error');  
            this.adaptiveCooldown();  
        }  
        ...
        this.activeContract = null;  
        // Unsubscribe from contract updates to avoid stale messages  
        this.sendMessage({  
            proposal_open_contract: 1,  
